...
// Filing copy, per destination. Hebrew localized.
const FILE_COPY: Record<FilableFolder, { done: string; failed: string }> = {
    archive: { done: "הועבר לארכיון", failed: "לא ניתן להעביר לארכיון" },
    trash: { done: "הועבר לאשפה", failed: "לא ניתן להעביר לאשפה" },
    inbox: { done: "הועבר לדואר נכנס", failed: "לא ניתן להעביר לדואר נכנס" },
};

function pluralHebrew(n: number): string {
    return n === 1 ? "שיחה אחת" : `${n.toLocaleString()} שיחות`;
}
...
export function useConversationActions(): ConversationActions {
    ...
    const offerUndo = React.useCallback(
        (message: string, threadIds: string[]) => {
            toast((t) => (
                <span className="flex items-center gap-3 text-[12.5px] text-slate-700 dir-rtl">
                    <span>{message}</span>
                    <button
                        type="button"
                        onClick={() => {
                            ...
                            moveFolderRequest({ threadIds, folder: "inbox" })
                                .then(() => {
                                    ...
                                    toast.success("הוחזר לדואר נכנס");
                                })
                                .catch(() => toast.error("לא ניתן לבטל"));
                        }}
    ...
    const setSeen = React.useCallback(
        (threadIds: string[], seen: boolean) => {
            ...
            if (threadIds.length > 1) {
                toast.success(
                    `${pluralHebrew(threadIds.length)} סומנו כ${seen ? "נקראו" : "לא נקראו"}`,
                );
            }
        },
        [markSeen],
    ...
    const snooze = React.useCallback(
        async (threadIds: string[], until: Date) => {
            ...
            if (
                ...
                toast.error("בחר מועד עתידי תקף");
                return;
            }
            if (until.getTime() - Date.now() > SNOOZE_MAX_MS) {
                toast.error("לא ניתן להשהות ליותר מ-90 יום");
                return;
            }
            ...
            try {
                ...
                toast.success(
                    threadIds.length === 1
                        ? "השיחה הושהתה"
                        : `הושהו ${pluralHebrew(threadIds.length)}`,
                );
            } catch {
                toast.error("לא ניתן להשהות");
            } finally {
                queryClient.invalidateQueries({ queryKey: ["unibox"] });
            }
        },
    ...
    const unsnooze = React.useCallback(
        async (threadIds: string[]) => {
            ...
            try {
                ...
                toast.success("ההשהיה בוטלה");
            } catch {
                toast.error("לא ניתן לבטל השהיה");
            } finally {
                queryClient.invalidateQueries({ queryKey: ["unibox"] });
            }
        },
