// The envelope of one message, opened in place from its header.
// Every value is a button that copies it. 100% Hebrew & RTL.
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Row label="אל">
                    {to.length > 0 ? <AddressList list={to} /> : <Muted>נמענים חסויים</Muted>}
                </Row>
                {cc.length > 0 && (
                    <Row label="עותק (Cc)">
                        <AddressList list={cc} />
                ...
                <Row label="נושא">
                    {subject ? (
                        <Copyable text={subject} what="נושא" className="break-words">
                            {subject}
                        </Copyable>
                    ) : (
                        <Muted>(ללא נושא)</Muted>
                    )}
                </Row>
                {mailbox ? (
                    <Row label="תיבת דואר">
                        <Copyable text={mailbox.email} what="כתובת" className="min-w-0">
                            {mailbox.name && mailbox.name !== mailbox.email && (
                                <span className="font-medium text-slate-800">{mailbox.name} </span>
                            )}
                            <span className={mailbox.name && mailbox.name !== mailbox.email ? "text-slate-500" : ""} dir="ltr">
                                {mailbox.email}
                            </span>
                        </Copyable>
                    </Row>
                    ...
                    <Row label="תיבת דואר">
                        <span className="inline-flex items-baseline gap-1.5 flex-wrap min-w-0">
                            <Muted>אינה מחוברת עוד</Muted>
                            <Copyable text={mailboxId} what="מזהה תיבה" mono>
                                {mailboxId}
                            </Copyable>
                        </span>
                    </Row>
                ) : null}
                {folder && <Row label="תיקייה">{folder}</Row>}
                {detail ? (
                    <>
                        {detail.message_id && (
                        ...
                        {detail.size > 0 && <Row label="גודל">{formatBytes(detail.size)}</Row>}
                    </>
                ) : loading ? (
                    <>
                        <SkeletonRow label="נשלח" width="w-[60%]" />
                        <SkeletonRow label="Message-ID" width="w-[70%]" />
                        <SkeletonRow label="גודל" width="w-12" />
                    </>
                ) : error ? (
                    <div className="col-span-2 flex items-center gap-1.5 text-[11.5px] text-amber-700">
                        <AlertCircleIcon className="w-3.5 h-3.5 shrink-0" />
                        לא ניתן היה לטעון את מלוא פרטי ההודעה.
                    </div>
                ) : null}
            </dl>
        </motion.div>
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
    return (
        <>
            <dt className="text-[10px] uppercase tracking-[0.14em] text-slate-400 leading-5 whitespace-nowrap text-right">
                {label}
            </dt>
            <dd className="min-w-0 leading-5 text-right">{children}</dd>
        </>
    );
}
...
function TimeValue({ date }: { date: Date }) {
    if (Number.isNaN(date.getTime())) return <Muted>לא ידוע</Muted>;
    return (
            ...
            {list.map((raw, i) => {
                ...
                return (
                    <li key={`${address}-${i}`} className="min-w-0 max-w-full">
                        <Copyable text={address} what="כתובת" className="min-w-0 max-w-full">
                            {name && <span className="font-medium text-slate-800 truncate">{name} </span>}
                            <span className={cn("truncate", name ? "text-slate-500" : "text-slate-800")} dir="ltr">
...
function Copyable({
    ...
    return (
        <button
            ...
            title={`העתק ${what}`}
            className={cn(
                "group/copy inline-flex items-center gap-1 max-w-full rounded px-1 -mx-1 text-right transition-colors hover:bg-slate-200/60",
                mono && "font-mono text-[11px] break-all dir-ltr text-left",
                className,
            )}
        >
            {children}
