...
const FOLDER_META: {
  ...
  { folder: "sent", label: "נשלח", icon: <SendIcon className="w-3.5 h-3.5 rtl:-scale-x-100" /> },
  {
    folder: "archive",
    label: "ארכיון",
  ...
  return (
    <nav className="h-full bg-slate-50/60 border-r border-slate-200 overflow-y-auto py-2 text-start">
      ...
      <Section label="תצוגות">
        ...
        />
        <Item
          icon={<ReplyIcon className="w-3.5 h-3.5 rtl:-scale-x-100" />}
          label="ממתין לתשובה"
        ...
        />
        <Item
          icon={<SendIcon className="w-3.5 h-3.5 rtl:-scale-x-100" />}
          label="מתוזמן"
      ...
      <CollapsibleSection
        ...
        emptyText={overview.isPending ? "טוען…" : "עדיין לא חוברו תיבות דואר."}
        searchPlaceholder="סינון תיבות דואר…"
      ...
      {data && data.categories && data.categories.length > 0 && (
        <CollapsibleSection
          ...
          searchPlaceholder="סינון קטגוריות…"
        ...
        <div className="px-1.5">
          ...
          {hidden > 0 && (
            <button
              ...
              className="w-full h-7 px-2 mt-1 rounded-md text-[11.5px] text-slate-500 hover:text-slate-900 hover:bg-white/70 transition-colors text-start rtl:text-right ltr:text-left"
            >
              הצג הכל (עוד {hidden})
            </button>
          )}
          {expanded && filtered.length > COLLAPSED_VISIBLE && (
            <button
              ...
              className="w-full h-7 px-2 mt-1 rounded-md text-[11.5px] text-slate-400 hover:text-slate-700 hover:bg-white/70 transition-colors text-start rtl:text-right ltr:text-left"
            >
              הצג פחות
            </button>
          )}
...
function ScheduledMeter({ used, cap }: { used: number; cap: number }) {
  ...
  return (
    <div className="px-1">
      ...
      >
        <span className="uppercase tracking-[0.14em] font-medium">תור</span>
      ...
      {ratio >= 0.95 && (
        <p className="mt-1 text-[10px] text-rose-600 leading-snug">
          קרוב למגבלה, בטל מספר שליחות כדי לפנות מקום.
        </p>
      )}
    </div>
  );
    ...
    >
      ...
      <PopoverMenu align="end">
        ...
        <PopoverMenuTrigger asChild>
          <button
            type="button"
            aria-label={`${label} פעולות תיקייה`}
            className={cn(
