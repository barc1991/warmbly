...
export default function AddSegmentToCampaignDialog({
    ...
    async function submit() {
        ...
        try {
            ...
            toast.success(
                res.added === 0
                    ? `כל החברים ב-${segment.name} כבר קיימים כלידים ב-${target?.name ?? "קמפיין"}`
                    : `נוספו ${res.added.toLocaleString()} ${res.added === 1 ? "ליד" : "לידים"} אל ${target?.name ?? "הקמפיין"}`,
            );
            onClose();
        } catch (err) {
            ...
            {open && (
                ...
                >
                    <motion.div
                        ...
                        role="dialog"
                        aria-modal="true"
                        aria-label="הוספת סגמנט לקמפיין"
                    ...
                    >
                        <header className="h-12 px-4 border-b border-slate-200 flex items-center gap-2.5 shrink-0">
                            ...
                            <span className="text-[10px] uppercase tracking-[0.14em] text-slate-400 font-medium">הוסף לקמפיין</span>
                            ...
                            <span className="hidden sm:inline-flex items-center h-5 px-1.5 rounded bg-sky-50 text-sky-700 text-[10px] font-medium">
                                {segment.contact_count.toLocaleString()} {segment.contact_count === 1 ? "איש קשר" : "אנשי קשר"}
                            </span>
                            <button
                                type="button"
                        ...
                        <div className="px-4 py-3 border-b border-slate-100 shrink-0">
                            <SearchInput value={query} onChange={setQuery} placeholder="חיפוש קמפיינים…" autoFocus className="w-full" />
                        </div>
                        <div className="flex-1 min-h-[200px] overflow-y-auto">
                            {campaigns.isPending ? (
                                ...
                                <div className="px-5 py-10 text-center">
                                    <p className="text-[12.5px] text-slate-900 font-medium">לא נמצאו קמפיינים</p>
                                    <p className="text-[11.5px] text-slate-400 mt-0.5">צור קמפיין תחילה, ולאחר מכן הוסף אליו סגמנט זה.</p>
                                </div>
                            ) : (
                                        ...
                                        return (
                                            <li key={c.id}>
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        className={cn(
                                                            "ms-auto shrink-0 text-[10.5px] uppercase tracking-[0.1em]",
                        ...
                        <footer className="px-3 py-2 min-h-12 border-t border-slate-200 flex flex-wrap items-center gap-x-2 gap-y-1.5 shrink-0 bg-slate-50/30">
                            <span className="text-[11px] text-slate-400 leading-snug basis-full sm:basis-0 sm:flex-1 sm:min-w-[160px] text-start">
                                מוסיף את החברים הנוכחיים. אנשי קשר שכבר קיימים בקמפיין ידולגו.
                            </span>
                            <button
                                type="button"
                                onClick={requestClose}
