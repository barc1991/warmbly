export default function Switch({id, value, onChange}:{id: string, value: boolean, onChange: (v: boolean) => void}) {
    return (
        <label htmlFor={id} dir="ltr" className="relative select-none inline-block w-11 h-6 cursor-pointer p-2 scale-95 shrink-0">
            ...
            <span className="absolute top-1/2 left-0.5 -translate-y-1/2 size-5 bg-white rounded-full shadow-xs transition-transform duration-200 ease-in-out peer-checked:translate-x-full"></span>
        </label>
    )
}