...
export function FirstEmailSection({
    ...
    return (
        <div className="space-y-4">
            <OptionSelect<Mode>
                aria-label="מועד שליחת האימייל הראשון"
                cols={2}
                value={mode}
                onChange={(v) => set(v === "now" ? 0 : minutes || lastWait)}
                options={[
                    { value: "now", label: "מיד עם ההצטרפות", hint: "יוצא בחלון השליחה הקרוב." },
                    { value: "wait", label: "לאחר המתנה", hint: "מתן זמן מסוים לכל איש קשר חדש תחילה." },
                ]}
            />
            ...
            <p className="text-[11px] leading-relaxed text-slate-400">
                נספר מרגע שכל איש קשר מצטרף, כך שמי שיתווסף בחודש הבא ימתין בדיוק אותו פרק זמן. זמני המתנה למיילי המשך מוגדרים בכל שלב. זוהי גם כרטיסיית הטריגר בראש לשונית השלבים.
            </p>
        </div>
    );
}
            ...
            <div className="flex items-start">
                ...
                <div className="relative flex-1 min-w-[72px] mt-3.5 mx-1">
                    ...
                    >
                        ...
                        {waits ? `ממתין ${entryDelayLabel(minutes)}` : "ללא המתנה"}
                    </span>
                </div>
                <TimelineStop
                    icon={<MailIcon className="w-3.5 h-3.5" />}
                    title="אימייל ראשון"
                    hint="בחלון השליחה הבא"
                    accent
                />
            </div>
        </div>
