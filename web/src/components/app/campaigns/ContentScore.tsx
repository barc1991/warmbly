...
function scoreTone(score: number) {
    if (score >= 80) return { text: "text-emerald-600", meter: "emerald" as DitherTone, label: "נראה מצוין" };
    if (score >= 50) return { text: "text-amber-600", meter: "amber" as DitherTone, label: "ניתן לשיפור" };
    return { text: "text-rose-600", meter: "rose" as DitherTone, label: "דורש שיפור" };
}
    ...
    return (
        <li className="flex items-start gap-2 py-1.5">
            ...
            <div className="min-w-0">
                ...
                <span className="ms-1.5 text-[10px] text-slate-400 font-mono">{issue.code}</span>
            </div>
        </li>
    );
}
        ...
        <div className="rounded-md border border-slate-200 bg-white">
            <div className="flex items-center justify-between gap-3 px-3 py-2.5">
                <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-slate-400 leading-relaxed">ציון עבירות מייעץ. אינו חוסם את השליחה.</p>
                </div>
                {pending && <Loading className="!w-3.5 h-3.5 shrink-0" />}
            </div>

            {failed && (
                <div className="px-3 pb-3 text-[11.5px] text-rose-600">לא ניתן היה לחשב ציון לתבנית זו.</div>
            )}
            ...
            {data && tone && (
                <div className="px-3 pb-3 border-t border-slate-200/60 pt-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={cn("ms-auto text-[11px] font-medium", tone.text)}>{tone.label}</span>
                    </div>
                    <DitherMeter
                    ...
                    {data.issues.length > 0 ? (
                        ...
                        <p className="mt-2 inline-flex items-center gap-1.5 text-[11.5px] text-emerald-600">
                            <ShieldCheckIcon className="w-3.5 h-3.5" /> לא נמצאו בעיות בתוכן.
                        </p>
                    )}
                </div>
            )}
