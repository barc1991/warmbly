        ...
        <div className="mt-1 space-y-0.5">
            ...
            {status?.kind === "similar" && (
                <p className="text-[10.5px] text-amber-700 flex flex-wrap items-center gap-x-1">
                    <AlertTriangleIcon className="w-3 h-3 shrink-0" />
                    <span>
                        כבר קיים שדה בשם <span className="font-medium">{status.existing}</span>.
                    </span>
                    <button
                        type="button"
            ...
            {sharedWith.length > 0 && (
                <p className="text-[10.5px] text-slate-500 leading-snug">
                    נכתב גם על ידי עמודה {sharedWith.join(", ")}. {sharedWith.length === 1 ? "כאשר לשתיהן יש ערך" : "כאשר למספר עמודות יש ערך"}, הערך מעמודה {kept} יישמר.
                </p>
            )}
        </div>
    );
...
export function TargetPicker({
    ...
    const label = isExisting ? customKey : isCustom ? (keyExists ? "שדה קיים" : "שדה חדש") : stdLabel ?? "התעלמות";
        ...
        <>
                ...
                <PopoverMenu align="start" open={open} onOpenChange={setMenuOpen}>
                    <PopoverMenuTrigger asChild>
                        <SelectButton
                            ...
                            label={label}
                            title={keyExists ? `שדה מותאם אישית קיים: ${customKey}` : undefined}
                    ...
                    <PopoverMenuContent minWidth={240} className="py-0 w-[260px]">
                        <div className="px-2 py-1.5 border-b border-slate-200">
                            <input
                                ...
                                placeholder={existingKeys.length > 0 ? "חיפוש או הגדרת שדה חדש..." : "חיפוש או הגדרת שדה..."}
                                autoFocus
                                aria-label="חיפוש שדות"
                                className="w-full h-5 bg-transparent text-[16px] md:text-[12px] text-slate-900 placeholder:text-slate-400 outline-none text-start rtl:text-right"
                            />
                        </div>
                        <div className="max-h-[min(60vh,360px)] overflow-y-auto py-1">
                            {standard.length > 0 && <PopoverMenuLabel>שדות סטנדרטיים</PopoverMenuLabel>}
                            {standard.map((t) => (
                                <PopoverMenuItem
                                    key={t.id}
                            ...
                            {custom.length > 0 && <PopoverMenuLabel>השדות המותאמים אישית שלך</PopoverMenuLabel>}
                            {custom.map((key) => {
                                    ...
                                    <PopoverMenuItem
                                        ...
                                        trailing={
                                            isCustom && customKey === key ? undefined : takenBy !== undefined ? (
                                                <span className="text-[10.5px] text-slate-400">עמודה {takenBy}</span>
                                            ) : undefined
                                        }
                                    >
                                        {key}
                            ...
                            <PopoverMenuLabel>חדש</PopoverMenuLabel>
                            {canCreate ? (
            ...
            {inferred && (
                <p className="mt-1 text-[10.5px] text-sky-700 inline-flex items-center gap-1">
                    ...
                    התאמה אוטומטית לפי משמעות העמודה. מומלץ לבדוק.
                </p>
            )}
            <MappingNote
                mapping={value}
