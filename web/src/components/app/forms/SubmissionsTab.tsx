...
function TriageChip({ triage, confidence }: { triage?: FormTriage; confidence?: number }) {
    ...
    const title = confidence ? `${TRIAGE_LABEL[triage]} (רמת וודאות ${Math.round(confidence * 100)}%)` : TRIAGE_LABEL[triage];
    return (
        <span
            title={title}
...
export default function SubmissionsTab({ form }: { form: Form }) {
    ...
    const filterTabs: { value: StatusFilter; label: string }[] = [
        ...
        ...(form.triage_enabled || counts.junk > 0 ? [{ value: "junk" as const, label: "ספאם" }] : []),
    ];
