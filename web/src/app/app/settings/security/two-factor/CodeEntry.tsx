...
// the caller wants an explicit button.
...
export default function CodeEntry({
    ...
    pending: boolean;
    /** הודעת שגיאה מתחת לשדה. שינוי מנקה את השדה */
    error: string | null;
    allowRecovery?: boolean;
    autoSubmit?: boolean;
    autoFocus?: boolean;
    /** מעדכן ערך כדי לאפשר כפתור בהורה */
        ...
        <div className="space-y-3">
            ...
            <div className="min-h-[18px] text-center">
                {pending ? (
                    <span className="inline-flex items-center gap-1.5 text-[12px] text-slate-400">
                        <Loader2Icon className="w-3.5 h-3.5 animate-spin" /> מאמת…
                    </span>
                ) : error ? (
            ...
            {allowRecovery && (
                <div className="text-center">
                    ...
                    >
                        {useRecovery ? "השתמש באפליקציית האימות במקום זאת" : "השתמש בקוד שחזור במקום זאת"}
                    </button>
                </div>
            )}
        </div>
