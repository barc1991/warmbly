...
export default function TeamsSettingsPage() {
    ...
    function doDelete(team: Team) {
        confirm?.show(`למחוק את הצוות "${team.name}"?`, async () => {
            try {
    ...
    return (
        <SectionShell
            title="צוותים"
            description={`קבץ חברים ב-${currentOrg?.name ?? "סביבת עבודה זו"} לצוותים מוגדרים.`}
        >
            <Section
                eyebrow="צור צוות"
                description="תן לצוות שם וצבע, ולאחר מכן הוסף חברים למטה."
            >
                <CreateTeamForm
            ...
            <Section
                eyebrow="צוותים"
                description={`${teams.length} ${teams.length === 1 ? "צוות" : "צוותים"} בסביבת עבודה זו.`}
            >
                {teamsQuery.isPending ? (
                    <p className="text-[11.5px] text-slate-400 py-2">טוען…</p>
                ) : teams.length === 0 ? (
                    <p className="text-[11.5px] text-slate-400 py-2">
                        אין צוותים עדיין. צור אחד למעלה.
                    </p>
                ) : (
            ...
            <div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <TextInput
                        value={name}
                        onChange={setName}
                        placeholder="מכירות, תמיכה, הנהלה…"
                        onKeyDown={(e) => {
                            if (e.key === "Enter") {
                                e.preventDefault();
                                submit();
            ...
            <div className="px-3 py-2.5">
                {team.members.length === 0 ? (
                    <p className="text-[11.5px] text-slate-400 py-1">
                        אין חברים עדיין. השתמש ב'הוסף חבר' כדי לשייך מישהו.
                    </p>
                ) : (
                    <div className="flex flex-wrap gap-1.5">
                        {team.members.map((m) => {
                            const label = m.name?.trim() || safeEmail(m.email) || `משתמש ${m.user_id.slice(0, 8)}`;
                            return (
                                <span
                                    key={m.user_id}
                                    className="group inline-flex items-center gap-1.5 h-6 pl-1 pr-1.5 rtl:pr-1 rtl:pl-1.5 rounded-full border border-slate-200 bg-slate-50 text-[11.5px] text-slate-700"
                                >
            ...
            <PopoverMenuContent minWidth={260} className="max-w-[calc(100vw-2rem)] p-1.5">
                ...
                {filtered.length === 0 ? (
                    <p className="px-2 py-3 text-[11.5px] text-slate-400 text-center">
                        {available.length === 0 ? "כולם כבר חברים בצוות זה." : "לא נמצאו תוצאות."}
                    </p>
                ) : (
                    <div className="max-h-56 overflow-y-auto">
                        {filtered.map((m) => {
                            const email = safeEmail(m.email) || `(משתמש ${m.user_id.slice(0, 8)})`;
                            return (
                                <button
                                    key={m.user_id}
                                    type="button"
                                    ...
                                    className="w-full px-1.5 py-1.5 rounded text-left rtl:text-right hover:bg-slate-100 transition-colors flex items-center gap-2"
                                >
                                    ...
                                    <span className="min-w-0">
                                        <span className="block text-[12px] text-slate-900 truncate leading-tight" dir="ltr">
                                            {email}
                                        </span>
                                        <span className="block text-[10px] text-slate-400 truncate font-mono leading-tight" dir="ltr">
                                            {m.user_id.slice(0, 8)}
                                        </span>
                                    </span>
                                </button>
