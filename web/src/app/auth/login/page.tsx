...
export default function LoginPage() {
    ...
    const handleLink = async (password: string) => {
        ...
        try {
            ...
            if (res.two_fa_required) {
                if (!res.pending_token) {
                    toast.error(isHe ? "משהו השתבש, נסה שוב." : "Something went wrong, please try again.");
                    return;
                }
                setPendingToken(res.pending_token);
                goTo("2fa");
            ...
            toast.success(
                isHe
                    ? `התחברות באמצעות ${providerLabel(linkChallenge.provider)} נוספה לחשבונך`
                    : `${providerLabel(linkChallenge.provider)} sign-in added to your account`,
            );
            await completeSession(res as unknown as Token);
        } catch (e) {
            toast.error(buildError(e as AppError));
    ...
    const providerLabel = (provider: string): string => {
        ...
        return authConfig.provider_labels?.oidc || (isHe ? "התחברות ארגונית (SSO)" : "single sign-on");
    };
    ...
    return (
        <div>
            <div className="text-center mb-6">
                <EmailPill email={email} onEdit={onBack} />
                <h1 className="text-[28px] font-bold text-slate-900 tracking-tight leading-tight">
                    {isHe ? "אמת שזה אתה" : "Confirm it's you"}
                </h1>
                <p className="text-sm text-slate-400 mt-1.5">
                    {isHe
                        ? `לכתובת זו כבר קיים חשבון ב-Warmbly. הזן את סיסמתך כדי לקשר התחברות באמצעות ${providerLabel}.`
            ...
            <form onSubmit={handleSubmit((data) => onSubmit(data.password))} className="space-y-4">
                <div>
                    <div className="flex items-center justify-between mb-1">
                        <label className="text-sm font-medium text-slate-600 ps-0.5">{isHe ? "סיסמה" : "Password"}</label>
                        <Link to="/auth/reset-password" className="text-xs text-sky-500 hover:text-sky-600 font-medium transition-colors">
                            {isHe ? "שכחת סיסמה?" : "Forgot password?"}
                        </Link>
                    </div>
                    <input
                        type="password"
                        placeholder={isHe ? "הזן את סיסמתך" : "Enter your password"}
                        className={INPUT}
                        autoComplete="current-password"
                        autoFocus
                        {...register("password")}
                ...
                <div className="pt-1">
                    <AuthButton loading={pending}>{isHe ? "קשר והתחבר" : "Link and sign in"}</AuthButton>
                </div>
            </form>

            <p className="text-xs text-slate-400 text-center mt-4">
                {isHe
                    ? "לא אתה? חזור אחורה והתחבר באמצעות סיסמה; דבר לא שונה בחשבונך."
