...
export function nameError(label: string, value: string, kind: NameKind, optional = false): string | null {
    ...
    if (!name) return optional ? null : `${label} הוא שדה חובה.`;
    const max = kind === "workspace" ? WORKSPACE_NAME_MAX : PERSON_NAME_MAX;
    if ([...name].length > max) return `${label} חייב להכיל עד ${max} תווים.`;
    let combining = 0;
    for (const ch of name) {
        if (FORBIDDEN_CHAR.test(ch)) return `${label} מכיל תווים שאינם מורשים.`;
        combining = COMBINING.test(ch) ? combining + 1 : 0;
        if (combining > MAX_COMBINING_RUN) return `${label} מכיל תווים שאינם מורשים.`;
    }
    if (looksLikeLink(name)) return `${label} לא יכול להכיל קישור, כתובת אתר או כתובת אימייל.`;
    const letter = kind === "workspace" ? /[\p{L}\p{N}]/u : /\p{L}/u;
    if (!letter.test(name)) {
        return kind === "workspace" ? `${label} חייב להכיל אות או מספר.` : `${label} חייב להכיל אות.`;
    }
    return null;
}
