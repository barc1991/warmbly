...
// "ישראל, דני ו-2 נוספים"; the address stands in for a missing name.
export function summarizeAddresses(list: string[], max = 2): string {
    ...
    return `${names.slice(0, max).join(", ")} ו-${names.length - max} נוספים`;
}
...
// Locale and zone of the reader, 24h Jerusalem time.
export function formatExactTime(d: Date, locale = "he-IL", timeZone = "Asia/Jerusalem"): string {
...
// Whole units of the largest that fits, in words: "לפני 3 ימים", "בעוד שעתיים".
export function relativeTime(d: Date, now = Date.now(), locale = "he-IL"): string {
