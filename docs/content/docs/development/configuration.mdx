...
| `WARMBLY_SENTRY_DSN` | web, admin containers | Browser error reporting, read at container start like the other `WARMBLY_*` values | unset |
...
| `APP_ENV` | every service | Doubles as the environment label on server-side events | `dev` |
...
An unset DSN means the SDK is never initialised: no transport thread, no host to contact, and nothing to opt out of. A set DSN can point at Sentry Cloud, a self-hosted Sentry, or any Sentry-compatible server such as GlitchTip; nothing in Warmbly assumes sentry.io.
