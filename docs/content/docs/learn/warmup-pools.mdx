...
When a workspace's paid subscription becomes active, Warmbly records the premium tier on every existing mailbox and moves its active warmup participants to the premium pool. Mailboxes connected later inherit the same tier. A workspace restricted or suspended for abuse keeps the paid tier on its mailboxes but cannot enter the premium participant pool until the restriction clears. Stripe webhook retries are safe, so an interrupted promotion finishes on redelivery without duplicating pool membership. Payment-recovery and cancellation events do not churn existing pool membership.
