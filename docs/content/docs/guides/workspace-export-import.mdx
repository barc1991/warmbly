...
| Campaigns | Campaigns, folders, sequences, senders, linked segments, attachments, the email image library, per-campaign settings, each lead's step progress with its per-link clicks and per-event opens, and the [unsubscribe links](/guides/unsubscribe/) already in recipients' inboxes |
...
A few things belong to the instance rather than the workspace and are left out on purpose: the instance's link to Warmbly Cloud and which mailboxes it enrolled in the hosted pool ([Warmbly Cloud](/guides/warmbly-cloud/)), linked self-hosted instances on a cloud workspace, live sessions, in-flight OAuth handshakes and the organization's wrapped data key. Reconnect to Warmbly Cloud after the move.

Reply and forward drafts kept in your browser are also excluded. Finish or copy them before moving to another instance. See [replying in Unibox](/guides/unibox/#replying).
