...
Turn on **Track opens and clicks on direct mail** in the mailbox's Settings tab to opt that mailbox in. From then on, replies and new messages you send from that mailbox get the same pixel and link tickets a campaign send gets, and the results appear under [Direct mail](/guides/analytics/#direct-mail) in Analytics.
