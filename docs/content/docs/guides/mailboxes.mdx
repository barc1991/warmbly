...
Verification proves DNS, not HTTPS, and the two are separate steps. On Warmbly Cloud the certificate is handled for you. On a self-hosted instance installed with the bundled Caddy it is handled too: Caddy obtains a certificate for your domain the first time a recipient loads a link on it. Behind an operator's own proxy it depends on that proxy, so if a domain verifies here but its links fail with a certificate warning, that is the thing to ask the operator about.
