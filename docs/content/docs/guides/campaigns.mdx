...
Follow-up spacing is separate and unaffected: that is each step's own wait, set on the connection into it (see [Sequences](/guides/sequences/#wait-and-spacing)). Leads that were already in the campaign before you set a delay count from the campaign's creation date, so turning one on does not re-delay people who have been enrolled for weeks. Changing the delay on a running campaign takes effect on save.
