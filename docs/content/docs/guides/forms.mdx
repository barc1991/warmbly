...
A visitor who opens the domain itself, with no form in the path, gets a short page saying the address serves forms and that each one has its own link. No form is listed there, and nothing on it identifies your workspace, so a customer who types the domain out of curiosity sees something deliberate rather than an error.
