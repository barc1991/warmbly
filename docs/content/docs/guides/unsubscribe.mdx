...
Unsubscribe links are served by the API process on the origin in `API_PUBLIC_URL`, so that variable must be set to an address recipients can reach. Leaving it unset does not disable the link: the backend falls back to its own listen address, which on a container is `http://localhost:8080`, and every opt-out in every campaign email points somewhere only that container can open. Links are signed under `AUTH_SECRET`; rotating it invalidates links in emails already sent.
