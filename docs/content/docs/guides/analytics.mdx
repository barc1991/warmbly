...
**Volume and replies** come from your synced mailboxes, so they cover every message the mailbox sent, including mail you wrote in Gmail or on your phone, and they cover history from before any of this existed. Nothing has to be switched on.
...
**Opens and clicks** are opt-in and measured differently. They come from the send records, so they only cover mail sent through Warmbly from a mailbox with tracking switched on, and only from the moment you switched it on. That is why the card states its own denominator rather than reusing "sent by hand" above: dividing opens by messages that were never asked would invent a rate.
...
**Per mailbox** ranks your mailboxes by how much each sent and received, and marks the ones that are tracking. **Top correspondents** ranks the people you exchange the most mail with. Your own mailboxes are excluded from that list, so a copy to yourself is not a correspondent.
