...
- **Auto-opens are labeled, not hidden.** Privacy proxies like Apple Mail Privacy Protection, and instant opens, still count (they confirm delivery) but are tagged and shown separately (`12 auto`). A stripped WebKit user agent is shared by Apple privacy fetching and Outlook image loading, so it is treated as a privacy prefetch only when it also arrives inside the automated-open window. A later request with that signature counts as a person's open, and its client is shown as **Image proxy** because the header cannot identify which service fetched it. **Auto-opens never trigger opened-based branches or automations.**
