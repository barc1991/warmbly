...
| Method | Path | API Permission |
|--------|------|----------------|
| GET | `/analytics/*` (dashboard, direct, inbox-tagging, deliverability, warmup, campaigns, accounts, usage) | `READ_ANALYTICS` |
...
`GET /analytics/inbox-tagging` accepts `limit`, opaque `cursor`, and `needs_review` query parameters and returns `data`, aggregate `summary`, and `pagination`. Invalid cursors, limits, or booleans return `400`.
