...
`errors` also carries notes about rows that were not failures, so an entry there does not always mean a lost row; a note about the import as a whole rather than one row carries `line: 0`. `segments_pinned` is present only when `segment_ids` was set: `true` when every membership write landed, `false` when one did not, with the reason among the notes.
